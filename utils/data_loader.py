...
def get_mean(args):
    mean = {
        ...
        "tinyimagenet": (
            ...
            0.4481,
            0.3975,
        ),  # (0.4914, 0.4822, 0.4465) https://github.com/AlbinSou/ocl_survey/blob/main/src/factories/benchmark_factory.py
...
def get_statistics(args: Config):
    ...
    std = {
        ...
        "tinyimagenet": (
            ...
            0.2265,
            0.2262,
        ),  #  (0.2023, 0.1994, 0.2010) https://github.com/AlbinSou/ocl_survey/blob/main/src/factories/benchmark_factory.py
...
def get_data(args: Config):
    ...
    if args.dataset_name == "mnist":
        train = datasets.MNIST(
            dir_data, train=True, download=True, transform=data_transforms
        )
        test = datasets.MNIST(
            dir_data, train=False, download=True, transform=data_transforms
        )
        val = None
    ...
    if args.dataset_name == "bloodmnist":
        train = medmnist.BloodMNIST(
            split="train", download=True, root=dir_data, transform=data_transforms
        )
        test = medmnist.BloodMNIST(
            split="test", download=True, root=dir_data, transform=data_transforms
        )
        val = medmnist.BloodMNIST(
            split="val", download=True, root=dir_data, transform=data_transforms
        )

    if args.dataset_name == "tissuemnist":
        train = medmnist.TissueMNIST(
            split="train", download=True, transform=data_transforms, root=dir_data
        )
        test = medmnist.TissueMNIST(
            split="test", download=True, transform=data_transforms, root=dir_data
        )
        val = medmnist.TissueMNIST(
            split="val", download=True, transform=data_transforms, root=dir_data
        )
    ...
    if args.dataset_name == "organamnist":
        train = medmnist.OrganAMNIST(
            split="train", download=True, transform=data_transforms, root=dir_data
        )
        test = medmnist.OrganAMNIST(
            split="test", download=True, transform=data_transforms, root=dir_data
        )
        val = medmnist.OrganAMNIST(
            split="val", download=True, transform=data_transforms, root=dir_data
        )

    if args.dataset_name == "organcmnist":
        train = medmnist.OrganCMNIST(
            split="train", download=True, transform=data_transforms, root=dir_data
        )
        test = medmnist.OrganCMNIST(
            split="test", download=True, transform=data_transforms, root=dir_data
        )
        val = medmnist.OrganCMNIST(
            split="val", download=True, transform=data_transforms, root=dir_data
        )

    if args.dataset_name == "organsmnist":
        train = medmnist.OrganSMNIST(
            split="train", download=True, transform=data_transforms, root=dir_data
        )
        test = medmnist.OrganSMNIST(
            split="test", download=True, transform=data_transforms, root=dir_data
        )
        val = medmnist.OrganSMNIST(
            split="val", download=True, transform=data_transforms, root=dir_data
        )

    if args.dataset_name == "tinyimagenet":
        train = datasets.ImageFolder(
            f"{args.dir_data}raw/tiny-imagenet-200/train", transform=data_transforms
        )
        test = datasets.ImageFolder(
            f"{args.dir_data}raw/tiny-imagenet-200/test", transform=data_transforms
        )
        val = datasets.ImageFolder(
            f"{args.dir_data}raw/tiny-imagenet-200/val", transform=data_transforms
        )
...
def get_balanced_yahoo(dataset, min_count):
    ...
    balanced_dataset = Dataset.from_dict(
        {
            "question_title": [
                example["question_title"] for example in balanced_dataset
            ],
            "question_content": [
                example["question_content"] for example in balanced_dataset
            ],
...
def get_combined_data_yahoo(dataset):
    ...
    for text in dataset:
        combined_text = (
            (text["question_title"] if text["question_title"] else "")
            + (" " + text["question_content"] if text["question_content"] else "")
            + (" " + text["best_answer"] if text["best_answer"] else "")
        )
...
def get_combined_data_dbpedia(dataset):
    combined_dataset = []
    for text in dataset:
        combined_text = (text["title"] if text["title"] else "") + (
...
def create_nlp_data(args):
    if args.dataset_name == "newsgroup":
        ...
        y_test = torch.stack(
            [torch.tensor(label, dtype=torch.int64) for label in newsgroups_test.target]
        )
    ...
    if args.dataset_name == "reuters":
        ...
        y_train = torch.stack(
            [torch.tensor(label, dtype=torch.int64) for label in train_labels]
        )

        input_test = ["query: " + news for news in decoded_test]
        y_test = torch.stack(
            [torch.tensor(label, dtype=torch.int64) for label in test_labels]
        )

    if args.dataset_name == "yahoo":
        train = load_dataset(
            "community-datasets/yahoo_answers_topics", split="train[:20000]"
        )
        test = load_dataset(
            "community-datasets/yahoo_answers_topics", split="test[:5000]"
        )
        ...
        y_train = torch.stack(
            [torch.tensor(text["topic"], dtype=torch.int64) for text in balanced_train]
        )
        y_test = torch.stack(
            [torch.tensor(text["topic"], dtype=torch.int64) for text in balanced_test]
        )
    ...
    if args.dataset_name == "dbpedia":
        ...
        input_test = ["query: " + text for text in combined_test]
        y_train = torch.stack(
            [torch.tensor(text["label"], dtype=torch.int64) for text in balanced_train]
        )
        y_test = torch.stack(
            [torch.tensor(text["label"], dtype=torch.int64) for text in balanced_test]
        )
...
def get_data_per_class(args):
    ...
    train_idx = (
        torch.nonzero(train_y[1:] != train_y[:-1], as_tuple=False)[:, 0] + 1
    ).tolist()
    test_idx = (
        torch.nonzero(test_y[1:] != test_y[:-1], as_tuple=False)[:, 0] + 1
    ).tolist()
    ...
    if val == None:
        ...
        val_idx = (
            torch.nonzero(val_y[1:] != val_y[:-1], as_tuple=False)[:, 0] + 1
        ).tolist()
    ...
    out_dict = {"train": train_ds, "val": val_ds, "test": test_ds}
    return out_dict
...
def get_loader_with_assignment(args, cls_assignment=None, run=None):
    ...
    if not os.path.exists(loader_fn):
        ...
        train_ds, val_ds, test_ds, cls_assignment = split_data_with_assignment(
            args, cls_assignment
        )
        ...
        for ds in ds_list:
            ...
            for task_data in ds:
                ...
                indices = torch.from_numpy(
                    np.random.choice(images.size(0), images.size(0), replace=False)
                )
                images = images[indices]
                label = label[indices]
                task_ds = torch.utils.data.TensorDataset(images, label)
                task_loader = torch.utils.data.DataLoader(
                    task_ds, batch_size=args.batch_size, drop_last=True
                )
    ...
    if not os.path.exists(loader_fn):
        ...
        for client_ds in all_clients_ds:
            ...
            for split_ds in client_ds:
                ...
                for task_data in split_ds:
                    ...
                    task_loader = torch.utils.data.DataLoader(
                        task_ds, batch_size=args.batch_size
                    )
                    loader_tmp.append(task_loader)
        ...
        for task_id in range(args.n_tasks):
            ...
            for client_id in range(len(loader_client_list)):
                test_task.append(
                    task_client_list[client_id][2][task_id]
                )  # [2] = test set

            concat_test_task = torch.utils.data.ConcatDataset(test_task)
            loader_test_task = torch.utils.data.DataLoader(
                concat_test_task, batch_size=args.batch_size
            )
