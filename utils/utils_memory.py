...
class Memory:
    def __init__(self, args):
        ...
        if args.dataset_name in ["newsgroup", "reuters", "yahoo", "dbpedia"]:
            self.memory_x = torch.FloatTensor(args.memory_size, args.input_size).fill_(
                0.0
            )
        else:
            self.memory_x = torch.FloatTensor(args.memory_size, *args.input_size).fill_(
        ...
        self.memory_y = torch.LongTensor(args.memory_size).fill_(0.0)
        self.memory_t = torch.Tensor(args.memory_size).fill_(0.0)
        ...
        affine = transforms.RandomAffine(
            degrees=20, translate=(0.1, 0.3), scale=(0.5, 0.75)
        )
        zoom = transforms.RandomResizedCrop(
            (28, 28), scale=(0.8, 1.0), ratio=(0.9, 1.1), antialias=True
        )
        augmentations = torch.nn.Sequential(
            flip, rotation, brightness, perspective, affine, zoom
        )
    ...
    def uncertainty_update(self, samples, labels, task_id, model):
        if (
            self.seen + self.args.batch_size <= self.args.memory_size
        ):  # fill the memory if spots are left
            for sample, label in zip(samples, labels):
                self.x[self.seen] = sample
                self.y[self.seen] = label
                self.t[self.seen] = task_id
            ...
            indices = torch.from_numpy(
                np.random.choice(self.x.size(0), size=500, replace=False)
            )
            mem_x_tmp, mem_y_tmp, mem_t_tmp = (
                self.x[indices],
                self.y[indices],
                self.t[indices],
            )
            ...
            mem_x_tmp, mem_y_tmp, mem_t_tmp = self.uncertainty_sampling(
                model,
                mem_candidates_x,
                mem_candidates_y,
                mem_candidates_t,
    ...
    def class_balanced_update(self, samples, labels, task_id, model, current_classes):
        ...
        if (
            ...
            mem_tmp_y = []
            mem_tmp_t = []
            mem_candidates_x = torch.cat(
                [self.x, samples.detach().cpu()]
            )  # concatenate current memory and mini-batch
            mem_candidates_y = torch.cat([self.y, labels.detach().cpu()])
            mem_candidates_t = torch.cat([self.t, task_ids])
            mem_per_class_count = sorted(
                Counter(mem_candidates_y.numpy()).most_common(), key=lambda tup: tup[1]
            )
            mem_used = 0
                    ...
                    if class_id in current_classes:
                        if mem_class_x.size(0) <= mem_per_class_tmp:
                            ...
                            if self.args.balanced_update == "uncertainty":
                                mem_class_x_tmp, mem_class_y_tmp, mem_class_t_tmp = (
    ...
    def random_sampling(self, subsample_size, exclude_task=None):
        ...
        if self.x.size(0) < subsample_size:
            ...
            indices = torch.from_numpy(
                np.random.choice(self.x.size(0), subsample_size, replace=False)
            )
            return self.x[indices], self.y[indices], self.t[indices]
            ...
            for cls in classes:
                ...
                indices = np.concatenate(
                    (
                        indices,
                        np.random.choice(
                            class_valid_indices, subsample_sum, replace=False
                        ),
                    ),
                    None,
                )
                    ...
                    if class_indices.size(0) < subsample_sum:
                        ...
                        indices = np.concatenate(
                            (
                                indices,
                                np.random.choice(
                                    class_indices, subsample_sum, replace=False
                                ),
                            ),
                            None,
                        )
    ...
    def uncertainty_sampling(
        ...
        unc_scores, descending_flag = compute_uncertainty_scores(
            self.args, mem_x, model, self.augmentations, seen_cls=self.seen_classes
        )
...
def compute_uncertainty_scores(
    args, mem_x, model, augmentations, tta_rep=5, seen_cls=None
):
    if args.dataset_name in ["newsgroup", "reuters", "yahoo", "dbpedia"]:
        ...
        transform_cands = [
            ...
            v2.RandomResizedCrop(
                args.input_size[1:], scale=(0.8, 1.0), ratio=(0.9, 1.1), antialias=True
            ),
            v2.RandomInvert(),
        ]
    ...
    if args.uncertainty_score == "rainbow":
        unc_scores, descending = rainbowSampling(
            logits_out, args, size=transformSize
        )  # size=tta_rep
